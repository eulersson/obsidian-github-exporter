...
export default class GitHubExporterPlugin extends Plugin {
	...
	async publishToGitHub() {
		try {
			...
			// Get all markdown files in the repository to check for attachments
			const allMarkdownFiles = new Set<string>();
			for (const [path, item] of existingFilesMap) {
				if (path.startsWith(this.settings.targetDir) && path.endsWith('.md')) {
					allMarkdownFiles.add(path);
				}
			}
			...
			// Get all attachments currently in use
	...
	getLinkedMedia(content: string): string[] {
		...
		const mediaRegex = /!\[\[([^\]]+\.(png|jpg|jpeg|gif|mp3|wav))\]\]/g;
